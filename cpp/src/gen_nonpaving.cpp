...
constexpr uint64_t kSeedStride = 1000003ULL;
      ...
      try {
        if (key == "field") {
          ...
          if (!ParseBoolValue(value, &parsed)) {
            if (err) {
              *err = "Invalid bool for generation.require_connected (expected true/false): " + value;
            }
            return false;
          }
        ...
        if (err) {
          *err = "Invalid value in config for key '" + key + "': " + ex.what();
        }
        return false;
      }
    }
...
bool ParseArgs(int argc, char** argv, GenConfig& cfg, std::string* err) {
  for (int i = 1; i < argc; ++i) {
    ...
    auto require_value = [&](const std::string& name) -> std::optional<std::string> {
      if (i + 1 >= argc) {
        if (err) {
          *err = "Missing value for argument: " + name;
        }
        return std::nullopt;
      }
    ...
    if (arg == "--config") {
      ...
      cfg.config_path = *val;
      if (!ApplyConfigFile(cfg, cfg.config_path, err)) {
        return false;
      }
    } else if (arg == "--field") {
      auto val = require_value(arg);
      ...
      std::cout
          << "Usage: gen_nonpaving [--config path] [--field 2|3] [--rank-min 4] [--rank-max 9]\n"
          << "                     [--n 10] [--threads N] [--seed S] [--max-seconds T] [--max-trials K]\n"
...
bool ValidateConfig(const GenConfig& cfg, std::string* err) {
  if (!(cfg.field == 2 || cfg.field == 3)) {
    if (err) *err = "--field must be 2 or 3";
    return false;
  }
  if (cfg.n != 10) {
...
std::vector<std::vector<int>> BuildSubsetsBySize(int n) {
  ...
  for (int mask = 1; mask < total; ++mask) {
    const int k = __builtin_popcount(static_cast<unsigned int>(mask));
    by_size[k].push_back(mask);
  }
  return by_size;
}
...
int RankSubset(const std::vector<std::array<uint8_t, kMaxN>>& cols, const std::vector<uint16_t>& cols_bits, int field,
  ...
  if (field == 2) {
    return RankGF2Subset(cols_bits, rank, subset_mask);
  }
  return RankGFqSubset(cols, field, rank, subset_mask);
}
...
std::vector<int> EnumerateBases(const std::vector<std::array<uint8_t, kMaxN>>& cols, const std::vector<uint16_t>& cols_bits,
                                int field, int rank, const std::vector<std::vector<int>>& subsets_by_size) {
...
bool IsConnectedMatroid(const std::vector<std::array<uint8_t, kMaxN>>& cols, const std::vector<uint16_t>& cols_bits,
  ...
  for (int mask = 1; mask < full_mask; ++mask) {
    ...
    const int r1 = RankSubset(cols, cols_bits, field, rank, mask);
    const int r2 = RankSubset(cols, cols_bits, field, rank, comp);
    if (r1 + r2 == rank) {
      return false;
    }
  }
...
void WorkerLoop(int worker_id, const GenConfig& cfg, const std::vector<std::vector<int>>& subsets_by_size, Stats& stats,
                SharedState& shared, std::ofstream& out_stream, std::atomic<uint64_t>& trial_counter,
                const std::chrono::steady_clock::time_point start_time) {
  std::mt19937_64 rng(cfg.seed + static_cast<uint64_t>(worker_id) * kSeedStride +
                      static_cast<uint64_t>(cfg.field) * kFieldStride);
  std::uniform_int_distribution<int> rank_dist(cfg.rank_min, cfg.rank_max);
  std::uniform_int_distribution<int> entry_dist(0, cfg.field - 1);
  const int full_mask = (1 << cfg.n) - 1;

  while (true) {
    const uint64_t trial = trial_counter.fetch_add(1);
    if (cfg.max_trials > 0 && trial >= cfg.max_trials) {
      return;
    }
    ...
    for (int c = 0; c < cfg.n; ++c) {
      for (int r = 0; r < rank; ++r) {
        cols[c][r] = static_cast<uint8_t>(entry_dist(rng));
      }
    }
    ...
    const int total_rank = RankSubset(cols, cols_bits, cfg.field, rank, full_mask);
    if (total_rank < rank) {
      continue;
    }
    ...
    auto bases = EnumerateBases(cols, cols_bits, cfg.field, rank, subsets_by_size);
    if (cfg.require_connected && !IsConnectedMatroid(cols, cols_bits, cfg.field, rank, cfg.n)) {
    ...
    bool is_new = false;
    {
      ...
      auto it = shared.canonical_seen.insert(canonical_label);
      is_new = it.second;
    }
    if (!is_new) {
    ...
    for (int c = 0; c < cfg.n; ++c) {
      matrix_cols.push_back(EncodeColumn(cols[c], cfg.field, rank));
    }

    const std::string id = HexU64(Fnv1a64(canonical_label));
    ...
    line << ",\"bases\":" << JsonIntArray(bases);
    line << ",\"non_paving_witness\":" << *witness;
    line << "}\n";
    ...
    stats.emitted_bases.fetch_add(static_cast<uint64_t>(bases.size()));
  }
}
...
int main(int argc, char** argv) {
  ...
  for (int worker = 0; worker < cfg.threads; ++worker) {
    workers.emplace_back(WorkerLoop, worker, std::cref(cfg), std::cref(subsets_by_size), std::ref(stats), std::ref(shared),
